...
#    append_json_to_file(list_df.to_json(orient='records',force_ascii=True),'./static/data/'+args[3]+'/'+args[4]+'/'+args[5]+'.json')
...
if __name__ == '__main__':
  ...
  for x in range(len(kyoten_id)):
    ...
    append_json_to_file(data_arr.to_json(orient='records',force_ascii=True),'./static/data/inter/day/'+args[1]+kyoten_id[x]+'.json')
  ...
  for x in range(len(kyoten_id)):
    ...
    data_arr = pd.merge(data_arr, temp_data_arr, on='title')
    data_arr = data_arr[data_arr['pagePath'].str.endswith('/')]
    append_json_to_file(data_arr.to_json(orient='records',force_ascii=True),'./static/data/dome/day/'+args[1]+kyoten_id[x]+'.json')
#  append_json_to_file(listArr.to_json(orient='records',force_ascii=True),'./static/data/inter/day/'+args[1]+kyoten_id[x]+'.json')

#  listArr = pd.DataFrame({},columns=['title','visits','pagePath'] )
#  for y in range(len(path_d)):
#    temp = query_list(kyoten_id[x],kyoten_name[x],path_d[y])
#    listArr = listArr.append( temp, ignore_index=True )
#  append_json_to_file(listArr.to_json(orient='records',force_ascii=True),'./static/data/dome/day/'+args[1]+kyoten_id[x]+'.json')
