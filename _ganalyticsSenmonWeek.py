    ...
    query_job = client.query("""
        WITH row_data AS (
            ...
            WHERE
              ...
              FORMAT_DATE("%Y%m%d", DATE_SUB(CURRENT_DATE('Asia/Tokyo'), INTERVAL EXTRACT(WEEK(MONDAY) FROM CURRENT_DATE('Asia/Tokyo'))+6 DAY))
              AND FORMAT_DATE("%Y%m%d", DATE_SUB(CURRENT_DATE('Asia/Tokyo'), INTERVAL EXTRACT(WEEK(SUNDAY) FROM CURRENT_DATE('Asia/Tokyo')) DAY))
        )
        SELECT
