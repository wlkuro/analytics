...
export default {
  ...
  mounted () {
    ...
    const dayAry = jsonList.filter(function(url){
      const pattern = new RegExp('static/data/dome/day/' + '([0-9]*)' + kyotenId + '.json', 'gi');
      return url.match(pattern);
    });
    const weekAry = jsonList.filter(function(url){
      const pattern = new RegExp('static/data/dome/week/' + '([0-9]*)' + kyotenId + '.json', 'gi');
      return url.match(pattern);
    });
    const monthAry = jsonList.filter(function(url){
      const pattern = new RegExp('static/data/dome/month/' + '([0-9]*)' + kyotenId + '.json', 'gi');
      return url.match(pattern);
    });
...
this.categories[0] = {id: 'day', name: 'day'}
  ...
  methods: {
    downloadCSV: function() {
      var csv = '\ufeff' + 'ページタイトル,URL,ページビュー\n'
      this.baseList.forEach(el => {
        var line = el['title'] + ',' + el['pagePath'] +',' + el['pageviews_y'] + '\n'
        csv += line
      })
    ...
    async senmon() {
      ...
      this.baseList.splice();
        ...
        const temp = await import(`~/static/data/dome/`+this.selectedCategory+`/`+this.selectedTag+query['kyoten']+`.json`);
